...
public class A2_123_LaminaBotones extends JPanel {
	public A2_123_LaminaBotones(String titulo, String[]opciones) {
		...
		setLayout(new BoxLayout(this,BoxLayout.Y_AXIS)); //ESTABLECEMOS DISPOSICIÓN DE LAMINA, VER LA API.
		grupo = new ButtonGroup();
		for(int i=0;i<opciones.length;i++) {
			JRadioButton bot = new JRadioButton(opciones[i]);
			bot.setActionCommand(opciones[i]); //ESTABLECE LA ACCIÓN DE COMANDO PARA EL BOTÓN.ES UN MÉTODO QUE JRADIOBUTTON HEREDA DE ABSTRACTBUTTON.	
		    add(bot);
		    grupo.add(bot);
		    bot.setSelected(i==0);//POR DEFECTO APARECERÁ LA 1RA OPCIÓN
		}
	}
	//METODO
