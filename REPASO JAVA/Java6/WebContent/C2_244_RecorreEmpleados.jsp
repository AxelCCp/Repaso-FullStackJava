...
<!-- ELIMINAMOS EL CAMPO BONUS Y SUMAMOS EL BONUS DIRECTAMENTE AL SALARIO -->
		...
		<c:forEach var="EmpTemp" items="${losEmpleados}" >
			<tr>
				<td>${EmpTemp.nombre}</td><td>${EmpTemp.apellido}</td><td>${EmpTemp.puesto}</td>
					<td>
						<c:if test = "${EmpTemp.salario<80000}">${EmpTemp.salario + 6000}</c:if>
						<c:if test = "${EmpTemp.salario>=80000}">${EmpTemp.salario}</c:if>
					</td>
