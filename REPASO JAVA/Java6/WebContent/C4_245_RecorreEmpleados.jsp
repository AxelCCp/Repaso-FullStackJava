...
<!-- USAMOS C:CHOOSE PARA:ENTREGAR BONUS DE 8000 A TRABAJADORES CON SUELDOS<40000 Y
	 ENTREGAR BONUS DE 2000 A TRABAJADORES CON SALARIOS ENTRE 4000 Y 50000 -->
		...
		<c:forEach var="EmpTemp" items="${losEmpleados}" >
			<tr>
				<td>${EmpTemp.nombre}</td><td>${EmpTemp.apellido}</td><td>${EmpTemp.puesto}</td>
					<td>
						<c:choose>
							<c:when test="${EmpTemp.salario<40000}">${EmpTemp.salario + 8000}</c:when>
